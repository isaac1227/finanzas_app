...
describe('Transacciones Component', () => {
  ...
  test('renderiza lista de transacciones correctamente', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Lista de Transacciones (2)')).toBeInTheDocument();
  ...
  test('permite crear nueva transacción', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('http://127.0.0.1:8000/transacciones', expect.objectContaining({
        method: 'POST',
  ...
  test('permite editar transacción existente', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('http://127.0.0.1:8000/transacciones/1', expect.objectContaining({
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  test('permite eliminar transacción con confirmación', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('http://127.0.0.1:8000/transacciones/1', {
        method: 'DELETE'
      });
    });
